...
def visualize_save_mueller(
        ...
        diagonal_range=(0, 1),  # <-- (vmin, vmax) for diagonal elements; use (-1, 1) for simulated data
        offdiagonal_range=(-0.1, 0.1),  # <-- (vmin, vmax) for off-diagonal elements
):
    """
    ...
    - diagonal_range (tuple, optional): (vmin, vmax) for diagonal elements. Defaults to (0, 1).
      Use (-1, 1) for simulated data where diagonal values can be negative.
    ...
    for i in range(16):
        ...
        if i in diagonal_indices:
            vmin, vmax = diagonal_range
        else:
            vmin, vmax = offdiagonal_range
...
def visualize_differential(diff_data, file_path, file_name='Differential.png', figsize=(15, 10), cmap='jet',
                           title='Differential Visualization', xlabel='', ylabel='', vmin=None, vmax=None):
    figure_path = Path(file_path)
    if not figure_path.exists():
        figure_path.mkdir(parents=True, exist_ok=True)
...
def visualize_save_mueller_with_mask(data, raw_path, visualisation_path, sample_number, wavelength, label="Sample",
                                     figsize=(15, 10), num_rows=600, num_cols=800, diagonal_range=(0, 1)):
    """
