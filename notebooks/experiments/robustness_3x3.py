...
XGB_PATH = file_paths.model_save_path / "pixel_xgb_3x3.json"
CAT_PATH = file_paths.model_save_path / "pixel_catboost_3x3.cbm"
MLP_PATH = file_paths.model_save_path / "best_pixel_mlp_3x3.pth"
OUT_DIR      = file_paths.experiment_results_path("3x3")
OUT_DIR.mkdir(parents=True, exist_ok=True)

# Held-out sample IDs (same as used in the 3x4 robustness testing).
...
def extract_3x3(arr_hw16: np.ndarray) -> np.ndarray:
    """Return the top-left 3×3 block of each 4×4 Mueller matrix.
    ...
    return M[:, :, :3, :3].reshape(-1, 9)


class PixelMLP(nn.Module):
    def __init__(self, in_features: int = 9, hidden_sizes: tuple = (128, 64, 32)):
        super().__init__()
...
def run_isolated_testing(xgb_model: xgb.Booster) -> pd.DataFrame:
    ...
    for tissue, sample_ids in ISOLATED_SAMPLES.items():
        ...
        for sid in sample_ids:
            ...
            X9 = extract_3x3(X16)          # (H*W, 9)
            y_pred = predict_xgb(xgb_model, X9)
    ...
    df_all.to_csv(OUT_DIR / "isolated_sample_metrics_3x3.csv", index=False)
    df_avg.to_csv(OUT_DIR / "isolated_sample_avg_metrics_3x3.csv", index=False)
...
def run_simulated_testing(
    ...
    """Run PR test on all simulated samples with all three 3×3 models."""
    candidates = [
    ...
    for fname in SIMULATED_FILES:
        ...
        X9       = extract_3x3(arr_norm)

        acc_xgb = accuracy_score(y_true, predict_xgb(xgb_model, X9))
        acc_cat = accuracy_score(y_true, predict_cat(cat_model, X9))
        acc_mlp = accuracy_score(y_true, predict_mlp(mlp_model, X9))
    ...
    if df.empty:
        ...
        return df
    df.to_csv(OUT_DIR / "simulated_metrics_3x3.csv", index=False)
